...
local function pl_draw_config_content(layout)
    ...
    for i, field in ipairs(pl_hex_fields) do
        ...
        G.playlog_hex_rects[i] = { x = cx, y = fy + 2, w = swatch_w, h = field_h - 4, key = field.key, label = field.label }
    end
end
...
local function pl_draw_panel(layout)
    ...
    if cslide < 0.99 then
        ...
        for i = first, last do
            ...
            if entry then
                ...
                local maybe_hovered, lines_used = pl_draw_rich_segments(entry.segments, layout.content_x, y, layout.content_x + layout.content_w - 10, seg_mx, my)
    ...
    if max_shift > 0 then
        ...
        love.graphics.rectangle("fill", layout.scrollbar_x, layout.scrollbar_y, layout.scrollbar_w, layout.scrollbar_h, 3, 3)
    ...
    if pl_is_run_active() then
        ...
        if pk and pk.hex_focus then
            ...
            elseif key == 'backspace' then
                ...
                local kp_map = {kp0='0',kp1='1',kp2='2',kp3='3',kp4='4',kp5='5',kp6='6',kp7='7',kp8='8',kp9='9'}
